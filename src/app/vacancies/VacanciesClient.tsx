...
function VacancyListItem({
  ...
  return (
    <motion.div
      ...
      className="bg-white rounded-lg p-4 border border-border/30 shadow-[0_2px_8px_rgba(60,60,120,0.08)] hover:shadow-[0_8px_25px_rgba(60,60,120,0.15)] transition-all duration-300 cursor-pointer w-full"
      onClick={() => onClick(vacancy)}
    >
      <div className={`flex ${imageUrl ? 'flex-col sm:flex-row gap-4' : 'flex-col'}`}>
        {/* Место для картинки слева - только если есть изображение */}
        {imageUrl && (
          <div className="flex-shrink-0 w-20 h-20 sm:w-24 sm:h-24 bg-gradient-to-br from-red-100 to-red-200 rounded-lg flex items-center justify-center mx-auto sm:mx-0 border-2 border-red-200/50">
        ...
        <div className="flex-1 min-w-0">
          <div className="flex flex-col sm:flex-row sm:justify-between sm:items-start mb-3 text-left">
          ...
          <div className="flex justify-end">
            <button className="px-4 py-2 bg-primary hover:bg-primary/90 text-white font-medium text-sm rounded-lg transition-colors shadow-sm">
              Подробнее
            </button>
          </div>
        </div>
